...
- 人和 AI 使用同一套简练、明确的信息：标题指出业务对象和问题，正文交代动作与结果，图形解释关系。关键业务含义保留在可选取的 HTML 标题、短文、列表或表格中；图内可编辑文字保留语义。不要把整篇内容图片化，也不要为抓取重复抄写整张图或扩写长文。
- 审稿时逐句检查具体指向：抽象价值词能否换成可观察的动作、字段、记录或处理结果；无法落到事实的句子删除。关键词、缩写及实体自然出现，不堆词、不重复定义，不添加为了 AI 引用而编造的 FAQ、结论或权威语气。
