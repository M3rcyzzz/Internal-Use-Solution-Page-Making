...
制作或修改任何方案页视觉时必读。默认把 Hero 产品图、场景示意、流程图、UNS 数据图、Builder 效果图和 Agent 演示做成 **Interactive Product Demo / Interactive Mockup**：用本地示例数据运行的可点击 HTML 界面模型。用户原始截图用于核对外观与事实，不再默认作为交付页的演示主体。

## 覆盖每个视觉位置

先列出当前页面所有视觉位置及其业务问题，逐个记录「初始状态 → 用户操作 → 可见结果 → 恢复方式」。交接清单包含每项的 DOM ID、语言、数据依据和实际验证结果；不能只改 Builder、遗漏 Hero 或其他场景图。

| 原视觉 | 界面模型至少完成的操作 |
|---|---|
| 应用截图、仪表盘、场景卡片 | 选择记录或筛选范围，打开对应详情；关键业务动作更新字段、列表或结果，可重置 |
...
Logo、装饰图标、真实现场照片以及用户明确要求保留的原始证据可保持静态。它们不能充当演示界面的替代品；保留来源和用途。新 HTML 给视觉容器标记 `data-visual="interactive|evidence|brand|decoration"`；交互模型同时使用唯一 `id` 和 `data-t0-demo`。证据例外写明 `data-evidence-reason`，不能通过改标签绕过实质检查。截图对照和 QA 截图可作为交接附件，不作为默认页面演示。
