...
def render(config):
    ...
    bv={**{k:text(b[k]) for k in ['heading','intro','prompt']},'app':app,
        'preview':render_app_demo(b['preview'],lang),'logo':url(config['logo']),
        'prompt_label':'Application requirements' if en else '应用构建需求',
        'builder_title':'Turn your ideas into applications' if en else '将需求变成应用',
        'copy_label':'Copy prompt' if en else '复制需求'}
    if b.get('highlight'):
    ...
    parts=[render_namespace(config['namespace'],sources,lang),
           Template((ROOT/'assets/components/builder.html').read_text()).substitute(bv),
           render_agent(a,lang)]
...
def main():
    ...
    try:
        ...
        assets=['components.css','components.js','app-demo.css','app-demo.js','service-modes.css']
